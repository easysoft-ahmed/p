...
import { Button, Dropdown, Form, Input, Menu, Select, Switch, Tree } from "antd";
                                ...
                                <td>
                                    <Select
                                        ...
                                        showSearch filterOption={(input, option) =>{
                                            ...
                                            let byProductName = (option?.ProductName ?? '').toLowerCase().includes(input.toLowerCase());
                                            return byProductName || byProductID
                                        }}
                                        onKeyDown={
                                            (e)=>F3(e,<TreeProduct handleEditRow={handleEditRow} tableName={"StockItems"} ele={ele}/>)
                                        }
                                        fieldNames={{value: "ProductID", label: "Productname"}}
                                        options={myData?.dataSelects?.products}
                                    />
                                    {/* <Select
                                        className="w-full"
                                        value={ele.ProductName || ele.ProductID ? myData?.dataSelects?.products?.filter(product => product.ProductID == ele.ProductID)[0]?.Productname || "الصنف غير متوفر ربما تم حذفة" : ele.ProductID}                                    
