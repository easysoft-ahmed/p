...
import {modified_tables_cash_in} from "../stateCashIn"
...
const Comp2 = ()=>{
    ...
    let handleEditRow = (tableName, actionType, fakeID, propsAndValue, closePopup)=>{
        dispatch(modified_tables_cash_in({tableName, actionType, fakeID, propsAndValue}))
        if(closePopup === "close"){
                    ...
                    <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                        <tr className="[&>*]:text-nowrap">
                            <th scope="col" class="px-2">
                                <button onClick={()=> dispatch(modified_tables_cash_in({tableName: "FinCashDets", data: defaultRowFinCashDets, actionType: "add"}))}><PlusOutlined /></button>
                            </th>
                    ...
                    {myData?.FinCashDets?.map(ele =>
                        <tr key={ele.fakeID}>
                            <td className="text-center">
                                <button onClick={()=> dispatch(modified_tables_cash_in({tableName: "FinCashDets", actionType: "remove", fakeID: ele.fakeID}))}><CloseCircleOutlined /></button>
                            </td>
                            <td>
                                <Select
                                    value={ele?.SideType}
