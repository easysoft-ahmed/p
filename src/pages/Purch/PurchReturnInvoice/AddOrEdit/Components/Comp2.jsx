...
import { edit_return_purch_invoice, modified_tables_return_purch_invoice, updateTotalValueAndTax } from "../stateReturnPurchInvoice";
...
const Comp2 = ()=>{
    ...
    let handleEditRow = (tableName, actionType, fakeID, propsAndValue, closePopup)=>{
        dispatch(modified_tables_return_purch_invoice({tableName, actionType, fakeID, propsAndValue}))
        if(closePopup === "close"){
    ...
    let handleDBclickRowCashes = (record, fakeID)=>{
        dispatch(modified_tables_return_purch_invoice({
                    ...
                    {myData?.PurchItems?.map(ele =>
                        <tr key={ele.fakeID}>
                            ...
                            <td className="text-center">
                                <button onClick={()=> dispatch(modified_tables_return_purch_invoice({tableName: "PurchItems", actionType: "remove", fakeID: ele.fakeID}))}><CloseCircleOutlined /></button>
                            </td>
                            <td>
                ...
                <div className="flex flex-wrap content-start w-6/12 p-4 border relative">
                    ...
                    <div className="flex flex-wrap justify-between input_label_basic px-4 w-6/12">
                        ...
                        <Select className="!w-8/12" value={myData?.PayType || 0}
                            onChange={value => {
                                dispatch(edit_return_purch_invoice({PayType: value, PurchCashes: [], PurchChecks: []}))
                                if (value === 0) {
                                    dispatch(modified_tables_return_purch_invoice({tableName: "PurchCashes", data: {...defaultRowPurchCashes, Value: myData?.NetTotal}, actionType: "add"}))
                                }
                                if (value === 3) {
                                    dispatch(modified_tables_return_purch_invoice({tableName: "PurchChecks", data: defaultRowPurchChecks, actionType: "add"}))
                                }
                        }}>
                    ...
                    {/* <div class="w-full relative overflow-x-auto mt-4">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                                <tr className="[&>*]:text-nowrap">
                                    <th scope="col" class="px-2">
                                        <button onClick={()=> dispatch(modified_tables_return_purch_invoice({tableName: "PurchCashes", data: defaultRowPurchCashes, actionType: "add"}))}><PlusOutlined /></button>
                                    </th>
                            ...
                            {myData?.PurchCashes?.map((ele) =>
                                <tr>
                                    <td className="text-center">
                                        <button onClick={()=> dispatch(modified_tables_return_purch_invoice({tableName: "PurchCashes", actionType: "remove", fakeID: ele.fakeID}))}><CloseCircleOutlined /></button>
                                    </td>
                                    <td>
                ...
                <div className="flex flex-wrap w-6/12 p-4 border relative">
                    ...
                    <div className="flex flex-wrap justify-between input_label_basic px-4 w-6/12">
                        ...
                        <Select
                            ...
                            onChange={(value, opt) => dispatch(edit_return_purch_invoice({TaxID: value, TaxRate: opt.TaxRate}))}
                            showSearch filterOption={(input, option) =>
                                (option?.label ?? '').toLowerCase().includes(input.toLowerCase())
                            }
        ...
        <div class="w-full relative overflow-x-auto mt-4">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                    <tr className="[&>*]:text-nowrap">
                        <th scope="col" class="px-2">
                            <button onClick={()=> dispatch(modified_tables_return_purch_invoice({tableName: "PurchChecks", data: defaultRowPurchChecks, actionType: "add"}))}><PlusOutlined /></button>
                        </th>
                ...
                {myData?.PurchChecks?.map((ele) =>
                    <tr>
                        <td className="text-center">
                            <button onClick={()=> dispatch(modified_tables_return_purch_invoice({tableName: "PurchChecks", actionType: "remove", fakeID: ele.fakeID}))}><CloseCircleOutlined /></button>
                        </td>
                        <td>
